/**
 * Test Automatisé du Module 7 : Pôle Studio & Médias
 * Validation des permissions RBAC, passerelle Cloud, récolte de clichés QR,
 * guides et balises data-tour.
 */
...
async function runTests() {
  console.log("=================================================");
  console.log("=== VÉRIFICATION MODULE 7 : PÔLE STUDIO =========");
  console.log("=================================================\n");
  ...
  // 1. Permissions RBAC
  console.log("--- 1. Permissions RBAC Pôle Studio ---");

  // Tiago (Membre standard) : accès refusé
  const tiagoAccessPole = canAccessPole('studio', TIAGO_PROFILE);
  if (!tiagoAccessPole) {
    results.permissions.success.push("Tiago (Membre) : Accès refusé au pôle 'studio'");
  } else {
    results.permissions.errors.push("ALERTE : Tiago a accès au pôle 'studio'");
  }

  STUDIO_TABS.forEach(tabId => {
    const hasTabAccess = canAccessTabPermission(tabId, 'studio', TIAGO_PROFILE);
    if (!hasTabAccess) {
  ...
  const camilaAccessPole = canAccessPole('studio', CAMILA_PROFILE);
  if (camilaAccessPole) {
    results.permissions.success.push("Camila (Admin) : Accès accordé au pôle 'studio'");
  } else {
    results.permissions.errors.push("ALERTE : Camila n'a PAS accès au pôle 'studio'");
  }

  STUDIO_TABS.forEach(tabId => {
    const hasTabAccess = canAccessTabPermission(tabId, 'studio', CAMILA_PROFILE);
    if (hasTabAccess) {
  ...
  // Profil Communication / Médias (Membre avec tag Communication)
  const comAccessPole = canAccessPole('studio', COM_PROFILE);
  if (comAccessPole) {
    results.permissions.success.push("Chargé de Com (Membre + Tag Communication) : Accès accordé au pôle 'studio'");
  } else {
    results.permissions.errors.push("ALERTE : Le profil avec tag 'Communication' n'a pas accès au pôle");
  }

  STUDIO_TABS.forEach(tabId => {
    const hasTabAccess = canAccessTabPermission(tabId, 'studio', COM_PROFILE);
    if (hasTabAccess) {
      results.permissions.success.push(`Chargé de Com : Accès accordé à l'onglet '${tabId}'`);
    } else {
      results.permissions.errors.push(`ALERTE : Le chargé de com n'a pas accès à l'onglet '${tabId}'`);
    }
  });
  ...
  // Guide général
  if (POLE_GUIDES['studio'] && POLE_GUIDES['studio'].etapes?.length > 0) {
    results.guides.success.push("Guide général 'studio' présent avec 3 étapes.");
  } else {
    results.guides.errors.push("Guide général 'studio' manquant.");
  }

  // Tous les 4 onglets doivent avoir un guide
  STUDIO_TABS.forEach(tabId => {
  ...
  const componentsToCheck = [
    { file: 'src/components/studio/StudioCloudHeader.jsx', attr: 'studio-cloud-root' },
    { file: 'src/components/studio/StudioEventsMediaTable.jsx', attr: 'studio-events-media-table' },
    { file: 'src/components/studio/StudioPhotosView.jsx', attr: 'studio-varal-photos-rope' }
  ];

  componentsToCheck.forEach(({ file, attr }) => {
    ...
    if (fs.existsSync(fullPath)) {
      ...
      results.dataTourAttributes.errors.push(`Fichier ${file} introuvable.`);
    }
  });

  // 4. Bilan Global
  console.log("\n=================================================");
  console.log("=== BILAN DU MODULE 7 : PÔLE STUDIO =============");
  console.log("=================================================");
  ...
  if (totalErrors === 0) {
    console.log("\n🎉 TOUS LES TESTS DU MODULE 7 SONT VALIDÉS AVEC SUCCÈS !");
    process.exit(0);
  } else {
