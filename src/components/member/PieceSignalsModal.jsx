...
/**
 * Modale Cordel des Signes du Mestre pour un morceau du Répertoire (< 180 lignes).
...
export default function PieceSignalsModal({
  ...
  const resolvedSignals = useMemo(() => {
    ...
    return list.sort((a, b) => Number(a.mesure) - Number(b.mesure));
            ...
            <div className="flex flex-col gap-3">
              <div className="flex items-center justify-between gap-2 p-2 rounded bg-white/80 border border-encre-noire/15 flex-wrap">
                ...
                <div className="flex items-center gap-2">
                  <button
                    type="button"
                    onClick={() => { setHideLabels(!hideLabels); setRevealedIds({}); }}
                    className="px-2.5 py-1 text-[11px] font-bold rounded border border-encre-noire/25 bg-stone-50 hover:bg-stone-100 text-stone-800 cursor-pointer"
                  >
                  ...
                  <button
                    type="button"
                    onClick={() => setActiveTab('quiz')}
                    className="px-3 py-1 text-[11px] font-black rounded bg-[var(--color-cordel-vert,#2d6a4f)] text-white hover:opacity-90 cursor-pointer shadow-2xs"
                  >
                    🎯 S'entraîner au Quiz
                  </button>
                </div>
